...
{
  if(minimum > nums[i]) 
	{ //goes through the array and checks if there are any elements smaller than the maximum
  minimum = nums[i];  //if the number is smaller than the maximum, the minimum variable becomes the element at that index
  	}
  if(maximum < nums[i])
	{  //goes through the nums array and checks if there are any values greater than the maximum
  maximum = nums[i]  //if the number is greater than the maximum, maximum becomes that element.
  	}
 }
sumOfValues = minimum + maximum;  //add the minimum and maximum, and set that to the sum variable
return sumOfValues; //return the final sum
}
